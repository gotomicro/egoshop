...
<!--										<view>{{item.cmtsNum}}评论</view>-->
		...
		<view :class="{'tui-order-list':scrollTop>=0}" v-else>
			<tui-tips :fixed="false" imgUrl="/static/images/toast/img_noorder_3x.png">暂无记录</tui-tips>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				currentTab: 0, // 0 全部，1 feed, 2 com
				pageIndex: 1,
				scrollTop: 0,
				listData: [],
				tid: 5, // 表示分享类型
		...
		methods: {
			...
			async getListData(first) {
				...
				const resp = await this.wxhttp.route(api.userGoods.list, {}, {tid:parseInt(this.tid),typeId:this.currentTab,currentPage:this.currentPage});
				if (resp.code !== 0) {
					uni.showToast({
						title: resp.msg,
						icon: "error",
