<template>
	<view class="container">
		<view class="tui-cmt-title">全部评论</view>
		<view class="tui-cmtbox">
			<view class="tui-cmt-cell" v-for="(item,index) in listData" :key="index">
	...
	export default {
		...
		methods: {
			async getListData(first) {
				const resp = await this.wxhttp.route(api.comment.list,{gid:parseInt(this.gid),typeId:parseInt(this.typeId),currentPage:this.currentPage});
				if (resp.code === 0 ) {
					this.totalPage = resp.data.total;
					if (first) {
						var comments = resp.data.list.map(function (item, index) {
							item.createdAt = relativeTime(item.createdAt)
							return item;
						})

						this.listData = comments;
					} else {
						var comments = resp.data.list.map(function (item, index) {
							item.createdAt = relativeTime(item.createdAt)
							return item;
						...
						//上拉加载跟多
						this.listData = this.listData.concat(comments);
					}
				}
			},
			...
			btnCmt: function() {
				uni.navigateTo({
					url:"/pages/commentCreate/main?gid="+this.gid+"&typeId="+this.typeId,
				})
			},
			cmtReply: function() {
				uni.navigateTo({
