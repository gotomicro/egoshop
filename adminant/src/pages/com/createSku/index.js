...
import { View } from "@/components/SaberFlexView/index";
...
@connect(({ comModel, loading }) => ({
    specList: comModel.comspecListData.list,
    specListLoading: loading.effects["comModel/comspecList"]
}))
export default class Sku extends Component {
            ...
            if (skus[0].spec.length > 0 && skus[0].spec[0]["id"] > 0) {
                ...
                skus.map((e) => {
                    e.spec.map((skuSpec) => {
                        ...
                        const specValueIndex = specs[specIndex].values.findIndex((value) => value.id === skuSpec.valueId);
                        if (specValueIndex === -1) {
                            specs[specIndex].values.push({
                                id: skuSpec.valueId,
                                name: skuSpec.valueName
                            });
                        }
                    });
                });
    ...
    customSpecModalOk = () => {
      if (this.AddSpecValueInput.input.value === "")   {
        message.error("不能为空")
        return
      }
      ...
      var that = this
        ...
        dispatch({
          type: "comModel/comspecCreate",
          payload: {
            name: this.AddSpecValueInput.input.value
          },
          callback: (resp) => {
            if (resp.code !== 0) {
              message.error("create is error, err is ",resp.msg)
              return
            }
            message.success("添加成功");
            that.setState({ loading: false, customSpecSortShow: false });
            that.initComSpecList();
          }
        })
    };
    ...
    addSpecValue = async ({ id, name }) => {
      const { dispatch } = this.props
      dispatch({
        type: "comModel/comspecValueCreate",
        payload: {
          specId:id,
          name: name,
        },
