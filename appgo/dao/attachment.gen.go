...
// Update 根据主键更新一条记录
func (g *attachment) Update(c *gin.Context, db *gorm.DB, paramId int, ups mysql.Ups) (err error) {
	var sql = "`id`=?"
...
func (g *attachment) Delete(c *gin.Context, db *gorm.DB, paramId int) (err error) {
	var sql = "`id`=?"
	var binds = []interface{}{paramId}
...
// Info 根据PRI查询单条记录
func (g *attachment) Info(c *gin.Context, paramId int) (resp mysql.Attachment, err error) {
	var sql = "`id`=?"
	var binds = []interface{}{paramId}
